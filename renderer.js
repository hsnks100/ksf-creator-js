...
KsfFile.prototype.setBegin = function(n) {
    beginSelect = n; 
}
KsfFile.prototype.setEnd = function(n) {
    endSelect = n; 
} 
KsfFile.prototype.clearSelection = function(n) {
    let e = endSelect; 
    let s = beginSelect;
    if (s > e) {
        // swap
    ...
    for(let j=s; j<=e; j++) {
        this.steps[j].div.classList.remove('selection-area');
    }
}
KsfFile.prototype.clearAllSelection = function(n) {
    for(let j=0; j<this.steps.length; j++) {
        this.steps[j].div.classList.remove('selection-area');
    }
}
KsfFile.prototype.drawSelection = function(n) {
    e = endSelect; 
    s = beginSelect;
    if (s > e) {
        // swap
    ...
    for(let j=s; j<=e; j++) {
        this.steps[j].div.classList.add('selection-area');
        // f.steps[j].div.focus();
    } 
}
KsfFile.prototype.setPosition = function(n) {
    beginSelect = endSelect = n;

    this.steps[n].div.focus(); 
    this.steps[n].div.scrollIntoView(); 
};

var files = [];

const selection = new SelectionArea({
                                    selectables: ['.step_row'],
                                    boundaries: ['#center'],
                                    startThreshold: 10,
                                    allowTouch: true,
                                    intersect: 'touch',
...
}).on('move', ({store, event, store: {changed: {added, removed}}}) => {
}).on('stop', evt => {
}); 
function addFile(binary, path) {
    ...
    for(let i=0; i<ret.length; i++) {
        ...
        if (rrr[0] == "TITLE") {
            ...
            stepDiv._dire_index = index;
            stepDiv.setAttribute("tabIndex", "0");
            stepDiv.classList.add('step_row');
            if (stepCount % 10 == 0) {
                stepDiv.classList.add('step_row_end');
            }
            // stepDiv.classList.add('selection-area');
            stepDiv.addEventListener("keydown", (e) => {
                if (e.code == "ArrowDown") {
                    f.clearSelection();
                    f.setPosition(endSelect + 1);
                    f.drawSelection();
                    ...
                    if (f.steps[index].unitStep[4] != '0') {
                        ...
                        f.steps[index].div.imgs[4].src = image3; // 'img/3.png';
                    } 
            ...
            stepDiv.addEventListener('mousedown', function() {
                // for(let j=0; j<f.stepData.length; j++) {
                //     f.stepData[j].div.style.backgroundColor = "#fff";
                // }
                // stepData.style.backgroundColor = "rgba(255, 0, 0, 0.3)";
                ...
                f.steps[index].div.classList.add('selection-area');
                console.log('mousedown index: ', index);
            });

            stepDiv.addEventListener('mouseup', function() {
                isSelecting = false;
                console.log('mouseup index: ', index);
            });
            stepDiv.addEventListener('mousemove', function() {
                ...
                // console.log('mousemove index: ', index);
            });
            ...
            for (var j=0; j<eachStep.length; j++) {
                ...
                if (eachStep[j] != '0') {
                    if(j == 0 || j == 5) {
                        fileName = image1; // 'img/1.png';
                    } else if(j == 1 || j == 6) {
                        fileName = image7; // 'img/7.png';
                    } else if(j == 2 || j == 7) {
                        fileName = image5; // 'img/5.png';
                    } else if(j == 3 || j == 8) {
                        fileName = image9; // 'img/9.png';
                    } else if(j == 4 || j == 9) {
                        fileName = image3; // 'img/3.png';
                    } 
                } else {
                    fileName = image0; // 'img/0.png';
                }
...
document.querySelector('#new_file').addEventListener('click', () => {
    ...
    remote.dialog.showOpenDialog().then(result => {
        ...
        // ""/home/dire/workspace/ksf-creator-js/aa.ksf""
...
let  tempRead = function(filepath) {
    window.fs.readFile(filepath, 'utf-8',(err, data) => {
        if(err){
            alert("An error ocurred reading the file :" + err.message);
            return;
...
tempRead("/home/dire/workspace/banya-Phantom/Crazy.ksf"); // ksf-creator-js/aa.ksf");
